...
def book(studio_id):
    ...
    status = "confirmed" if not STRIPE_SECRET_KEY else "pending_payment"
    created_at = datetime.now(TZ).strftime("%Y-%m-%d %H:%M:%S")
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO bookings (studio_id, full_name, email, phone, date, start_time, end_time, status, notes, created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?)
    """, (studio_id, full_name, email, phone, selected_date.isoformat(), start_time_str, end_t.strftime("%H:%M"), status, notes, created_at))
